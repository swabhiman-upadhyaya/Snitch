...
const categoryColors = {
  Apparel:     'bg-pink-50 text-pink-800',
  Home:        'bg-blue-50 text-blue-800',
  Accessories: 'bg-green-50 text-green-800',
  Kitchen:     'bg-yellow-50 text-yellow-800',
  Stationery:  'bg-cyan-50 text-cyan-800',
};
...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen font-sans" style={{ background: 'linear-gradient(160deg,#fdf8f6 0%,#f7eff4 100%)', fontFamily: "'Inter',system-ui,sans-serif" }}>

      {/* ── Nav ── */}
      <nav className="flex items-center justify-between px-4 sm:px-8 h-[52px]" style={{ background: '#49243E' }}>
        <div className="flex items-center gap-2.5">
          <div className="w-[26px] h-[26px] rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: '#DBAFA0' }}>
        ...
        <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-semibold" style={{ background: '#BB8493' }}>
          S
        </div>
      </nav>
      ...
      <div className="max-w-[1100px] mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-6">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.07em] mb-1" style={{ color: '#BB8493' }}>Dashboard</p>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-6">
            {stats.map((stat, i) => (
              <div
                ...
                style={{ border: `1px solid ${stat.accent ? '#fad4d4' : '#f0e6ec'}` }}
              >
                <div
                  className="w-8 h-8 sm:w-9 sm:h-9 rounded-[9px] flex items-center justify-center flex-shrink-0"
                  style={{
                    background: stat.accent ? '#fdf0f0' : '#DBAFA030',
                    color: stat.accent ? '#c0392b' : '#704264',
                  }}
                >
                  {stat.icon}
                </div>
                <div className="min-w-0">
                  <p className="text-[9px] sm:text-[10px] font-semibold uppercase tracking-[0.05em] mb-0.5 truncate" style={{ color: '#704264' }}>
                    {isMobile
                  ...
                  <p
                    ...
                    style={{ color: stat.accent ? '#c0392b' : '#49243E' }}
                  >
                    {stat.value}
                  </p>
                </div>
        ...
        {sellerProducts && sellerProducts.length > 0 && (
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* View toggle */}
              <div className="flex rounded-[9px] overflow-hidden bg-white flex-shrink-0" style={{ border: '1px solid #ead8e4' }}>
                {[
                  {
                    key: 'grid',
                  ...
                  <button
                    ...
                    className="w-9 h-[36px] flex items-center justify-center transition-colors"
                    style={{
                      background: viewMode === v.key ? '#DBAFA050' : 'transparent',
                      color: viewMode === v.key ? '#49243E' : '#BB8493',
                      border: 'none',
                      cursor: 'pointer',
                    }}
                  >
        ...
        {sellerProducts && sellerProducts.length > 0 ? (
          filtered.length === 0 ? (
            <div className="bg-white rounded-2xl p-12 text-center" style={{ border: '1px solid #f0e6ec' }}>
              <p className="font-semibold text-[15px] mb-1" style={{ color: '#49243E' }}>No products found</p>
              <p className="text-[13px]" style={{ color: '#BB8493' }}>Try a different search term.</p>
            </div>
          ) : viewMode === 'grid' ? (
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              {filtered.map(product => (
                <ProductCard key={product._id} product={product} onDelete={() => setDeleteModal(product)} />
              ))}
            </div>
          ) : (
            <div className="flex flex-col gap-2.5">
              {filtered.map(product => (
                <ProductRow key={product._id} product={product} onDelete={() => setDeleteModal(product)} isMobile={isMobile} />
              ))}
            </div>
          )
        ) : (
          ...
          <div className="bg-white rounded-[20px] py-20 px-6 text-center" style={{ border: '1px solid #f0e6ec' }}>
            <div
              ...
              style={{ background: '#DBAFA030' }}
            >
              <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#704264" strokeWidth="1.6">
                <path d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
              </svg>
            </div>
            <h3 className="text-lg font-bold tracking-tight mb-2" style={{ color: '#49243E' }}>No products yet</h3>
            <p className="text-sm mb-6 max-w-xs mx-auto leading-relaxed" style={{ color: '#BB8493' }}>
      ...
      {deleteModal && (
        <div
          ...
          style={{ background: 'rgba(73,36,62,0.35)' }}
          onClick={() => !deleting && setDeleteModal(null)}
        >
          <div
            className="bg-white rounded-2xl p-6 w-full max-w-sm text-center"
            style={{ boxShadow: '0 8px 40px rgba(73,36,62,0.18)' }}
          ...
          >
            ...
            <h3 className="text-base font-bold tracking-tight mb-1.5" style={{ color: '#49243E' }}>Delete product?</h3>
            <p className="text-[13px] leading-relaxed mb-5" style={{ color: '#BB8493' }}>
              <span className="font-semibold" style={{ color: '#704264' }}>{deleteModal.title}</span> will be permanently removed from your store.
            </p>
            <div className="flex gap-2.5">
              <button
                onClick={() => setDeleteModal(null)}
                disabled={deleting}
                className="flex-1 h-[38px] rounded-[10px] text-[13px] font-semibold bg-white cursor-pointer transition-colors"
                style={{ border: '1px solid #ead8e4', color: '#704264' }}
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleting}
                className="flex-1 h-[38px] rounded-[10px] text-[13px] font-semibold text-white border-none transition-colors"
                style={{ background: deleting ? '#e8b4b4' : '#c0392b', cursor: deleting ? 'not-allowed' : 'pointer' }}
              >
...
function ProductCard({ product, onDelete }) {
  ...
  const catClass = categoryColors[product.category] || 'bg-pink-50 text-pink-800';
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden bg-[#faf5f8]" style={{ aspectRatio: '4/3' }}>
        ...
        {outOfStock && (
          <div className="absolute bottom-0 left-0 right-0 text-white text-[10px] font-semibold text-center py-1.5 uppercase tracking-wider" style={{ background: 'rgba(73,36,62,0.82)' }}>
            Out of stock
          </div>
        )}
      </div>
      ...
      <div className="p-3.5">
        ...
        <div className="flex items-center justify-between mb-0">
          {product.category
            ? <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-md ${catClass}`}>{product.category}</span>
            : <span />}
        ...
        <div className="flex items-center justify-between pt-2.5 mt-2.5" style={{ borderTop: '1px solid #f5eff3' }}>
          ...
          <div className="flex gap-1.5">
            ...
            <ActionBtn icon="trash" danger onClick={onDelete} />
          </div>
        </div>
      </div>
    </div>
...
/* ─── Product Row ──────────────────────────────────────────────── */
function ProductRow({ product, onDelete, isMobile }) {
  const catClass = categoryColors[product.category] || 'bg-pink-50 text-pink-800';
  const outOfStock = product.stock === 0;

  return (
    <div
      className="bg-white rounded-2xl flex items-center gap-3 px-3 py-3 sm:px-4 cursor-pointer transition-all"
      style={{ border: '1px solid #f0e6ec' }}
      onMouseEnter={e => (e.currentTarget.style.borderColor = '#d4a8bc')}
      onMouseLeave={e => (e.currentTarget.style.borderColor = '#f0e6ec')}
    >
      {/* Thumb */}
      <div className="w-12 h-12 rounded-xl overflow-hidden bg-[#faf5f8] flex-shrink-0">
        {product.images?.[0] ? (
          <img src={product.images[0].url} alt={product.title} className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#DBAFA0" strokeWidth="1.5">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-semibold truncate mb-0.5" style={{ color: '#49243E' }}>{product.title}</p>
        <p className="text-[11px] truncate" style={{ color: '#BB8493' }}>{product.description}</p>
      </div>

      {/* Category — hidden on mobile */}
      {!isMobile && product.category && (
        <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-md flex-shrink-0 ${catClass}`}>
          {product.category}
        </span>
      )}

      {/* Price */}
      <span className="text-[13px] font-bold flex-shrink-0" style={{ color: '#49243E' }}>
        {getCurrencySymbol(product.price?.currency)}{product.price?.amount?.toLocaleString()}
      </span>
      ...
      {!isMobile && (
        <span
          ...
          style={{
            color: outOfStock ? '#c0392b' : '#704264',
            background: outOfStock ? '#fdf0f0' : '#DBAFA025',
          }}
        >
      ...
      {!isMobile && (
        <span className="text-[11px] flex-shrink-0" style={{ color: '#BB8493' }}>
...
function ActionBtn({ icon, danger, onClick }) {
  ...
  const color = danger ? '#c0392b' : '#704264';

  const icons = {
    view: (
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={hov ? color : '#BB8493'} strokeWidth="2">
  ...
  return (
    <button
      ...
      className="w-7 h-7 rounded-[7px] `flex items-center justify-center transition-all p-0"
      style={{
        border: `1px solid ${hov ? color + '40' : '#ead8e4'}`,
